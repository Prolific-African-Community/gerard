...
The public contract is exported by `@prolific/gerard-core`. Supported write actions are `updateIdentity`, `updateBranding`, `updateModules`, `updateIntegrationConfig` and `updateIntegrationEnabled`; read actions are `getConfiguration` and `getOrgAdmins`; ORG_ADMIN recovery actions are `resetOrgAdminPassword`, `invalidateOrgAdminSessions` and `reactivateOrgAdmin` (payload `{ userId }` only, SUPER_ADMIN only). Payloads are allow-listed; passwords, API keys, webhook secrets, secret references, database URLs, model names and arbitrary organization IDs are never accepted.
