...
function parseBody(body: unknown): ClientProfilePayload | null {
  ...
  return {
    name: getOptionalString(payload.name) ?? undefined,
...
async function handler(
  ...
  if (!id) {
    return res.status(400).json({ error: "Client profile id is required" });
  }
  ...
  if (req.method === "GET") {
    try {
      ...
      return res.status(200).json({ clientProfile: toResponse(profile) });
    } catch (error) {
      console.error("Failed to load client profile", error);
      return res
        .status(500)
        .json({ error: "Impossible de charger le profil client." });
    }
  }
  ...
  if (req.method === "PUT") {
    ...
    try {
      const uniquenessError = await validateUniqueClientProfile(body, id);
      ...
      return res
        .status(500)
        .json({ error: "Impossible de modifier le profil client." });
    }
  }
  ...
  if (req.method === "DELETE") {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Impossible de désactiver le profil client." });
    }
  }
