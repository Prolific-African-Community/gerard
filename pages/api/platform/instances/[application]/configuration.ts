...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Reads follow the platform read-only policy (SUPER_ADMIN and PLATFORM_SUPPORT); every write stays SUPER_ADMIN only.
  ...
  if (!application || (!isRead && !isPlatformConfigurationAction(action)) || !payload || typeof payload !== 'object' || Array.isArray(payload)) return res.status(400).json({ error: 'Invalid configuration request' })
