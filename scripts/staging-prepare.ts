...
// Prisma CLI run by this Node binary without a shell; its output (which names the database host) is redacted before it
// reaches build or operator logs. A failure keeps the redacted output and names its stage.
const prismaCli = path.join(process.cwd(), 'node_modules', 'prisma', 'build', 'index.js')
function prismaStep(args: string[], stage: string) {
  console.log(`staging-prepare: prisma ${args.join(' ')}`)
  try {
    process.stdout.write(redact(execFileSync(process.execPath, [prismaCli, ...args], { env: process.env, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], shell: false })))
  } catch (error) {
    const failed = error as { stdout?: string; stderr?: string; message?: string }
    process.stdout.write(redact(`${failed.stdout ?? ''}${failed.stderr ?? ''}${failed.stdout || failed.stderr ? '' : failed.message ?? ''}\n`))
    throw new Error(stage)
  }
}

async function main() {
  // Self-contained: the Prisma Client is generated here (never assumed from an earlier build), then migrations, then
  // the client is loaded and the QA accounts are ensured.
  prismaStep(['generate'], 'STAGING_PRISMA_GENERATE_FAILED')
  prismaStep(['migrate', 'deploy'], 'STAGING_MIGRATION_FAILED')
  console.log('staging-prepare: QA accounts')
  ...
  try {
    ...
    throw new Error(`STAGING_PRISMA_RUNTIME_FAILED: ${error instanceof Error ? error.message : String(error)}`)
  }
  try {
    await ensureAccounts(prisma, hashPassword)
  } catch (error) {
    throw new Error(`STAGING_QA_BOOTSTRAP_FAILED: ${error instanceof Error ? error.message : String(error)}`)
  } finally {
    await prisma.$disconnect()
  }
}

async function ensureAccounts(prisma: typeof import('../lib/prisma')['prisma'], hashPassword: typeof import('../lib/auth/password')['hashPassword']) {
  if (custom) {
...
main().catch((error) => { console.error(redact(`staging-prepare failed: ${error instanceof Error ? error.message : 'unknown'}`)); process.exit(1) })
