...
export function productionFixture() {
  ...
  return {
    teams: [{ id: 'team_gerard', slug: 'prolific' }],
    projects: [
...
export async function startMockVercel(state) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/v2/teams') return send(200, { teams: state.teams })
    if (url.searchParams.get('teamId') !== 'team_gerard') return send(404, { error: { code: 'not_found', message: 'scope' } })
    ...
    switch (route) {
      case 'GET v9/': return send(200, { id: project.id, name: project.name, accountId: project.accountId, protectionBypass: project.protectionBypass })
      case 'GET v9/custom-environments': return send(200, { environments: project.customEnvironments })
  ...
  return { url: `http://127.0.0.1:${server.address().port}`, state, writes, close: () => new Promise((resolve) => { server.close(resolve); server.closeAllConnections() }) }
}
